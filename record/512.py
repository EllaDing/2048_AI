    ...
    for x in range(grid.size):
        for y in range(1, grid.size):
            ...
            if grid.map[x][y-1] is 0:
                ...
                credit += 400 * (grid.map[x][y-1]**3)

            elif grid.map[x][y-1]/grid.map[x][y] is 2:
                credit += 200 * (grid.map[x][y-1]**2)
...
def score(grid):
   ...
   # print (m, sp, 10 * grid.getMaxTile())
    score = m + 800/space + d * 50 * (grid.getMaxTile()**2) + grid.getMaxTile() * grid.getMaxTile()
